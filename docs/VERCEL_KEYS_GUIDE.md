...
### 🏨 8. 订酒店/火车票/机票（携程联盟）
没有这个，"Book a hotel/transport"功能会显示我们精选的几家酒店信息 + 一个跳转到 Trip.com 官网的按钮（能用，只是不是携程给你专属返佣的链接）。这个需要**申请通过审核**才能拿到 key，不是注册就有，可能需要等待人工审核。

| 变量名 | 填什么 | 去哪里申请 |
|---|---|---|
| `CTRIP_UNION_API_KEY` | 携程联盟分配给你的 App Key | [union.ctrip.com](https://union.ctrip.com) 申请成为联盟会员（需要审核） |
| `CTRIP_UNION_API_SECRET` | 携程联盟分配的签名密钥 | 同上，审核通过后联盟后台能看到 |
| `CTRIP_UNION_PID` | 携程联盟给你的推广位 ID（用来统计你的佣金） | 同上 |
