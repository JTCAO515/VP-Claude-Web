...
### 🏨 8. 订酒店/火车票/机票（携程）
**这一组已经默认能用，通常不需要你动手填。** 携程联盟把"调用接口查询"的方式取消了，
现在改成了一个"生成链接小工具"——不需要账号审核，系统已经内置了一组默认的推广 ID，
"Book a hotel/transport"功能开箱即用，点击后会跳转到携程官网对应的搜索结果页。

| 变量名 | 填什么 | 去哪里获取 |
|---|---|---|
| `CTRIP_AID` | 携程的推广渠道 ID（系统已有默认值，一般不用改） | 如果你之后注册了自己的携程联盟账号，可以在后台换成你自己的 ID，这样产生的订单佣金会算到你账上 |
| `CTRIP_SID` | 携程的子推广位 ID（系统已有默认值，一般不用改） | 同上 |

⚠️ 注意：携程这边目前是用"自动拼接网址"的方式实现跳转，不是正式调用接口查询数据，
所以网址里的参数命名**还没有用真实携程账号验证过**。如果你发现跳转后页面不对，
告诉负责接手的程序员，让他对照携程开放平台的"URL生成工具"重新核实参数名称。
