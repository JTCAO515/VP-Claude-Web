...
| 不足 | 为什么没做 | 如果要做，怎么做 |
|---|---|---|
| 携程的对接方式中途变了一次 | 携程联盟**取消了原来可调用的查询接口**，改成"URL生成工具"——不需要账号审核，直接按规则拼接 H5 链接（`api/partners.py` 里的 `_ctrip_url`），系统已内置默认的 `CTRIP_AID`/`CTRIP_SID`。**但链接里具体的参数名称还没有用真实账号验证过**，是按携程公开文档反推的最佳猜测。 | 找一个真实携程联盟账号，在他们后台的"URL生成工具"里手动生成 4 种类型（酒店列表/酒店详情/火车票/机票）各一条示例链接，对照 `api/partners.py` 里的参数名核实，不一致就改。 |
...
2. **核实携程 H5 链接参数 + 申请美团联盟账号**——携程那边不用等审核，但需要找真实账号验证链接参数对不对；美团联盟有审核周期，越早申请越好，不然团购预订功能一直停留在"精选数据+通用链接"阶段。
