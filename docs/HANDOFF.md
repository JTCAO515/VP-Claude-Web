...
**最后更新**：2026-06-25，对应版本 v8.2.x，commit 范围从 v7.0.0 重写到当前。
...
产品负责人这轮提了 4 件事，全部已完成并部署：

1. **三平台接入能力说明 + 接入携程联盟/美团联盟/高德评分**（v8.2.0，上一轮）——已经写在 CHANGELOG 里，这里不重复。
2. **Vercel key 填写指南**（小白向）→ `docs/VERCEL_KEYS_GUIDE.md`。
3. **这份 HANDOFF 文档**。
4. **合并 Plan + Trips 为一个 tab**，**Cities 详情页加酒店/餐厅/景点三分类浏览**——见下方"已知状态"，这些改动应该已经在这次commit里落地（如果你在读这份文档时这两项还没做，说明执行被打断了，请检查 git log 确认到哪一步）。
...
| 不足 | 为什么没做 | 如果要做，怎么做 |
|---|---|---|
| 携程联盟/美团联盟 API 的真实请求签名格式**未经验证** | 这两个平台需要申请审核才能拿到账号，我没有真实账号去对照最新文档调试。代码里是 best-effort 实现 + 明确注释。 | 等产品负责人申请到账号后，把 `api/partners.py` 里 `_ctrip_hotel_search` / `_meituan_deal_search` 两个函数对照官方最新文档重新核对字段名和签名算法。 |
...
2. **申请携程联盟/美团联盟账号**——这两个有审核周期，越早申请越好，不然预订功能一直停留在"精选数据+通用链接"阶段。
